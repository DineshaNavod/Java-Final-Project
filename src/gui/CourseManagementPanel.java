...
public class CourseManagementPanel extends JPanel {
    ...
    public void loadCourses() {
        ...
        try {
            ...
            for (CourseUnit c : courses) {
                model.addRow(new Object[]{c.getCCode(), c.getCName(), c.getCredit(), c.getIsTheory(), c.getIsPractical()});
            }
    ...
    private void deleteSelected() {
        ...
        int c = JOptionPane.showConfirmDialog(this, "Delete course: " + name + "?",
    ...
    private void showCourseDialog(CourseUnit existing) {
        ...
        JComboBox<String> theoryBox   = new JComboBox<>(new String[]{"YES", "NO"});
        JComboBox<String> practBox    = new JComboBox<>(new String[]{"YES", "NO"});
        ...
        saveBtn.addActionListener(e -> {
            ...
            if (code.isEmpty() || name.isEmpty() || creditStr.isEmpty()) {
                JOptionPane.showMessageDialog(dialog, "All fields are required."); return;
            }
            try {
                ...
                if (existing == null) {
                    courseDAO.addCourse(new CourseUnit(code, name, credit, th, pr));
                } else {
                    existing.setCName(name); existing.setCredit(credit);
                    existing.setIsTheory(th); existing.setIsPractical(pr);
                    courseDAO.updateCourse(existing);
                }
                loadCourses();
                JOptionPane.showMessageDialog(dialog, "Course saved successfully!");
                dialog.dispose();
